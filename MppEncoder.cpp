...
extern std::string globalstreamType;
	...
	*length = 0;
    ...
    /*        if (mpp_packet_is_partition(packet)) {
            ...
            if (mpp_packet_has_meta(packet)) {
                ...
                if (MPP_OK == mpp_meta_get_s32(meta, KEY_LONG_REF_IDX, &lt_idx))
                    printf(" lt %d", lt_idx);
                if (MPP_OK == mpp_meta_get_s32(meta, KEY_ENC_FRAME_QP, &avg_qp))
                    printf(" qp %d", avg_qp);
            }
            mpp_packet_deinit(&packet);
            p->stream_size += len;
...
MPP_RET MppEncoder::test_mpp_setup_legacy(MpiEncData *p) {
	...
	if (p->fps_in_num == 0) p->fps_in_num = 60;
	if (p->fps_out_den == 0) p->fps_out_den = 1;
	if (p->fps_out_num == 0) p->fps_out_num = 60;
	 p->gop = 60;
...
MPP_RET MppEncoder::test_mpp_enc_cfg_setup(MpiEncData *p) {
	...
	/* fix input / output frame rate */
	printf(" 帧率 ---- %d    ----   %d \n", p->fps_in_num, p->fps_in_den);

	printf(" 输出帧率 ---- %d    ----   %d \n", p->fps_out_num, p->fps_out_den);
    ...
    case MPP_ENC_RC_MODE_CBR : {
        ...
        mpp_enc_cfg_set_s32(cfg, "rc:bps_min",  p->bps*15 / 16);
			printf(" walker ---CBR-------配置 %d---最小 %d---\n",p->bps_max ? p->bps_max : p->bps * 17 / 16,p->bps_min ? p->bps_min : p->bps * 15 / 16);
    } break;
    ...
    case MPP_ENC_RC_MODE_AVBR : {
		...
		//p->bps
		// printf(" walker ---111----码率 %d----配置 %d---最小 %d---\n", p->bps,p->bps_max ? p->bps_max : p->bps * 17 / 16,p->bps_min ? p->bps_min : p->bps * 1 / 16);
		// mpp_enc_cfg_set_s32(cfg, "rc:bps_max", 2048);
        // mpp_enc_cfg_set_s32(cfg, "rc:bps_min", 2048-500);
...
void MppEncoder::setUp(int width, int height, int bps,int profile ,int fps) {
	...
	//args_.type = MPP_VIDEO_CodingHEVC;
