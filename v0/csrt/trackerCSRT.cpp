...
static std::vector<double> preApv;
static std::vector<double> preResMax;
static int simFailCnt;
static int failCntThres = 2;
static float m_conf;
static bool m_isLost;
static cv::Mat opth, cpth;
static float sw;
...
public:
    bool update()
    {
        double ssim = calculateSSIM(opth, cpth);
        ...
        if((ssim > 0.7 || sw > 30.f) || m_setupf++ < 5)
        // if(hsim < 0.2)
        {
            m_oriPatch = cpth.clone();
            // trackerPtr->updateRoi(image);
            m_stpUpdt = 0;
        }
        else
        {
            if(m_stpUpdt++ > 30 && ssim > 0.5 && sw > 30.f)
            {
    #if TRACKER_DEBUG
...
public:
    ...
    virtual cv::Rect find(cv::Mat, float &sim);
    virtual double getConf();
    virtual void setObjPt(cv::Point pt);
...
Point2f TrackerCSRTImpl::estimate_new_position(const Mat &image)
{
    ...
    double max_val;
    double minValue;
    minMaxLoc(resp, &minValue, &max_val, NULL, &max_loc);
    ...
    for(int i=0; i<resp.cols; i++){
		...
		value_diff = resp.at<float>(i,j) - minValue;
		sum_diff += std::pow(value_diff,2); 
	}
    }
    ave_res = sum_diff/(resp.cols*resp.rows);
    double mmdiff=0;
    mmdiff = max_val-minValue;
    mmdiff = std::pow(mmdiff,2);
    apv=mmdiff/ave_res;

    preApv.push_back(apv);
    preResMax.push_back(max_val);
    ...
    if(sz>0) {
        ...
        if (apv < comApv && max_val < comResMax) 
            simFailCnt++;
        else
            simFailCnt = 0;

        if(simFailCnt > failCntThres)
            m_isLost = true;
    ...
    printf("apv:%f, comApv:%f, max_val:%f, comResMax:%f, m_isLost:%d\n", apv, comApv, max_val, comResMax, m_isLost);
...
cv::Rect TrackerCSRTImpl::find(cv::Mat frame, float &sim)
{
    // cv::Rect ret;
    // object_center = estimate_new_position(frame);
    Mat resp = calculate_response(frame, csr_filter);
...
double TrackerCSRTImpl::getConf()
{
    return m_conf;
}

void TrackerCSRTImpl::setObjPt(cv::Point pt)
{
    object_center = pt;
}
