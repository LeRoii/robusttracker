...
static int detfindfailedcnt = 0;
#define CAL_VELO_MODE 2
...
static const std::vector<cv::Scalar> predefinedColors = {
    cv::Scalar(255, 0, 0),  // 表示蓝色 (BGR)
    cv::Scalar(0, 255, 0),  // 表示绿色 (BGR)
    cv::Scalar(0, 0, 255),   // 表示红色 (BGR)
    cv::Scalar(255, 255, 0), // 表示青色 (BGR)
    cv::Scalar(255, 0, 255), 
    cv::Scalar(0, 255, 255), 
    cv::Scalar(255, 127, 0),
    cv::Scalar(127, 0, 255), 
...
realtracker::realtracker(std::string cfg)
{
    ...
    trackerCfg.detDist = config["detDist"].as<double>();
    trackerCfg.withdet = config["withdet"].as<int>();
    ...
    m_csrtTracker = cv::TrackerCSRT::create();
    ...
    tracker = std::make_unique<BoTSORT>("/userdata/3/bin/tracker.ini");
...
void realtracker::init(const cv::Point &pt, cv::Mat &trackImage, cv::Mat &detImage)
{
    ...
    bool find = false;
    cv::Rect finalroi = cv::Rect{pt.x - m_stracker->m_GateSize/2, pt.y - m_stracker->m_GateSize/2, m_stracker->m_GateSize, m_stracker->m_GateSize};

    if(trackerCfg.withdet ==1)
    {
        PushImg(trackImage);
        usleep(30000);
        std::pair<cv::Mat, std::vector<bbox_t>> detret;
        int num = getRet(detret);
    
        printf("detret size:%d\n", num);
    
        double finaldist = 1000;
                
    
        for(auto& ret:detret.second)
        {
            cv::Point p1(ret.x+ret.w/2, ret.y+ret.h/2);
            double dist = getDistance(p1, pt);
            printf("dist:%f\n", dist);
            if(dist < finaldist)
            {
                finaldist = dist;
                finalroi = cv::Rect(ret.x, ret.y, ret.w, ret.h);
            }
        }
    
        if(finaldist < trackerCfg.detDist)
        {
            find = true;
        }
        else
        {
            if(detfindfailedcnt++ < 2)
            {
                m_state = EN_TRACKER_FSM::INIT;
                m_initPt = pt;
                printf("detfindfailedcnt++ < 2\n");
                return;
            }
            else
            {
    ...
    printf("detfindfailedcnt++aaaaa > 2\n");
    ...
    if(trackerCfg.useCSRT == 1)
    {
        ...
        if(find)
        {
            cv::Point centerp = cv::Point(finalroi.x + finalroi.width/2, finalroi.y + finalroi.height/2);
            // roi= finalroi;
            roi = cv::Rect(centerp.x - finalroi.width/2*0.8, centerp.y - finalroi.height/2*0.8, finalroi.width*0.8, finalroi.height*0.8);
        }
        else
        {
            roi= cv::Rect{pt.x - m_stracker->m_GateSize/2, pt.y - m_stracker->m_GateSize/2, m_stracker->m_GateSize, m_stracker->m_GateSize};
        }
        ...
        m_csrtTracker->init(trackImage, roi);
    }
...
void realtracker::FSM_PROC_STRACK(cv::Mat &frameDetect, cv::Mat &frameTracker, cv::Rect &trackRect)
{
   // printf("\nFSM_PROC_STRACK\n");
    ...
    else
    {
        ...
        m_state = EN_TRACKER_FSM::STRACK;
        m_trackObj.update(frame, m_strackerRet, trackerCfg.useCSRT == 1 ? m_csrtTracker->getConf() : m_stracker->getConf());
    }
...
//     }
    ...
    if (m_ssearchCnt++ < trackerCfg.ssearchCntThres)
    {
        ...
        if(trackerCfg.useCSRT == 1)
        {
            m_csrtTracker->setObjPt(m_trackObj.center());
        }
        else
            m_stracker->setRoi(m_trackObj.m_rect);
        trackRect = m_trackObj.m_rect;
    ...
    if(m_ssearchCnt % 12 == 0)
    {
        ...
        if(trackerCfg.useCSRT == 1)
        {
            ...
            float f;
            rect = m_csrtTracker->find(frame, f);
            sim = f;
...
void realtracker::fsmUpdate(cv::Mat &frameDetect, cv::Mat &frameTracker, cv::Rect &trackRect)
{
    ...
    case EN_TRACKER_FSM::SSEARCH:
        ...
        break;
    case EN_TRACKER_FSM::INIT:
        init(m_initPt, frameTracker, frameTracker);
        break;
    default:
        break;
    }
...
// #if TRACKER_DEBUG
    ...
    if (m_state == EN_TRACKER_FSM::DTRACK || m_state == EN_TRACKER_FSM::STRACK || m_state == EN_TRACKER_FSM::SSEARCH)
    {
        ...
        if(offsetAbnormalCnt > 80)
        {
            x = 0;
            y = 0;
            m_state = EN_TRACKER_FSM::LOST;
            offsetAbnormalCnt = 0;
            trackRect = cv::Rect(0,0,0,0);
        }
    ...
    else if(m_state == EN_TRACKER_FSM::SSEARCH && m_ssearchCnt > 60 && trackerCfg.withServo == 0)
    {
...
void realtracker::runTracker(cv::Mat &frame, bool alone)
{
    if(trackerCfg.useCSRT == 1)
    {
        ...
        bool isfound = m_csrtTracker->update(frame, roi);
        // printf("isfound:%d\n", isfound);
 ...
 * @param tracks Tracks
 */
void realtracker::plot_tracks(cv::Mat &frame, std::vector<std::shared_ptr<Track>> &tracks)
//void realtracker::plot_tracks(cv::Mat &frame, std::vector<std::shared_ptr<Track>> &tracks)
{
    static std::map<int, cv::Scalar> track_colors;
    cv::Scalar detection_color = cv::Scalar(0, 0, 0);
    cv::Scalar ID_color = cv::Scalar(255, 0, 0);
        // printf(" realtracker::plot_tracks 数量 %d\n ",tracks.size());
    for (const std::shared_ptr<Track> &track: tracks)
    {
        std::vector<float> bbox_tlwh = track->get_tlwh();
        // cv::Scalar color = cv::Scalar(rand() % 255, rand() % 255, rand() % 255);
       
       cv::Scalar color = predefinedColors[track->_class_id];
        ...
        cv::putText(frame, std::to_string(track->track_id),
                    cv::Point(static_cast<int>(bbox_tlwh[0]),
                              static_cast<int>(bbox_tlwh[1])),
                    cv::FONT_HERSHEY_SIMPLEX, 0.75, ID_color, 2);
...
void realtracker::PushImg(cv::Mat &frame)
{
    if (!m_irFrame)
    {
        //m_detector->ImgInference(frame, detRet, boxs_count);
        m_detector->putImg(frame);
    }
    else
    {
        m_irDetector->putImg(frame);
    }
}

int realtracker::getRet(std::pair<cv::Mat, std::vector<bbox_t>> &detret)
{
    if (!m_irFrame)
    {
        //m_detector->ImgInference(frame, detRet, boxs_count);
        return m_detector->getRet(detret);
    }
    else
    {
        //m_irDetector->ImgInference(frame, detRet, boxs_count);
        return m_irDetector->getRet(detret);
    }
}
void realtracker::clear()
{
    if (!m_irFrame)
    {
        m_detector->clear();
...
{
    return (m_state == EN_TRACKER_FSM::SSEARCH && m_ssearchCnt > 80);
}
