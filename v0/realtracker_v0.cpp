...
realtracker::realtracker(std::string enginepath) : m_frameInfo()
{
    ...
    // m_detector = new idetector(const_cast<char *>(enginepath.c_str()), 1);
    // m_detector->init();
...
void realtracker::runDetector(cv::Mat &frame, cv::Mat &pre_Frame, std::vector<TrackingObject> &detRet)
{
    printf("realtracker::runDetector\n");
    std::vector<bbox_t> boxs;

    m_frameInfo.m_frames[0].GetMatBGRWrite() = pre_Frame.clone();
    // m_frameInfo.m_frames[0].GetMatBGRWrite() = frame.clone();
    // m_detector->process(frame, boxs);
    // for (auto i = boxs.begin(); i != boxs.end(); i++)
    // {
    //     rectangle(pre_Frame, cv::Point(i->x, i->y), cv::Point(i->x + i->w, i->y + i->h), cv::Scalar(255, 0, 0), 2);
    // }
    ...
    // pre_Frame = m_frameInfo.m_frames[0].GetMatBGR();
#endif
}
