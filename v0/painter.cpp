...
void PaintRollAngleAxis(cv::Mat &frame0, double currRollAngle)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        cv::putText(frame0, Convert(tempPos), cv::Point(x - 10, yStart - 35), cv::FONT_HERSHEY_SIMPLEX, stSysStatus.osdFontSize, cv::Scalar(0, 255, 0), fontThickness, cv::LINE_AA);
        tempPos += 5;
        x += interval;
        for (int j = 0; j < 4; j++) {
            cv::line(frame0, cv::Point(x, yStart - 5), cv::Point(x, yStart - 12), cv::Scalar(0, 255, 0), fontThickness, cv::LINE_AA);
...
void PaintPitchAngleAxis(cv::Mat &frame0, double currPitchAngle)
{
    ...
    cv::putText(frame0, currStr, cv::Point(xStart + 30, yStart), cv::FONT_HERSHEY_SIMPLEX, stSysStatus.osdFontSize, cv::Scalar(0, 255, 0), fontThickness, cv::LINE_AA);
...
void PaintCrossPattern(cv::Mat &frame0, float currRollAngle, float currPitchAngle)
{
    ...
    // 绘制x线
    cv::line(frame0, cv::Point(x - lineLen, y), cv::Point(x - lineLen / 4, y), cv::Scalar(0, 255, 0), fontThickness, cv::LINE_AA);
    cv::line(frame0, cv::Point(x + lineLen, y), cv::Point(x + lineLen / 4, y), cv::Scalar(0, 255, 0), fontThickness, cv::LINE_AA);

    // 绘制中心点
    cv::circle(frame0, cv::Point(x, y), 2, cv::Scalar(0, 255, 0), fontThickness, cv::LINE_AA);

    // 绘制y线
    cv::line(frame0, cv::Point(x, y - lineLen), cv::Point(x, y - lineLen / 4), cv::Scalar(0, 255, 0), fontThickness, cv::LINE_AA);
    cv::line(frame0, cv::Point(x, y + lineLen), cv::Point(x, y + lineLen / 4), cv::Scalar(0, 255, 0), fontThickness, cv::LINE_AA);
}
...
static void PaintDegMinSec(cv::Mat &frame0, int x, int y, const double inputAngle, bool isLongitude)
{
    ...
    cv::putText(frame0, currDeg, cv::Point(x + 15, y), cv::FONT_HERSHEY_SIMPLEX, stSysStatus.osdFontSize, cv::Scalar(0, 255, 0), fontThickness, cv::LINE_AA);
}
...
void PaintCoordinate(cv::Mat &frame0)
{
    ...
    if (!stSysStatus.isTSeriesDevice && stSysStatus.osdSet2Ctrl.enTAGGPSShow) {
        ...
        cv::putText(frame0, tagAlt, cv::Point(x + 40, y + 90), cv::FONT_HERSHEY_SIMPLEX, stSysStatus.osdFontSize, cv::Scalar(0, 255, 0), fontThickness, cv::LINE_AA);
    }
...
void PaintViewPara(cv::Mat &frame0)
{
    ...
    if (stSysStatus.osdSet1Ctrl.enEOFieldOfViewOrMultiplyShow) {
        ...
        if (!stSysStatus.isTSeriesDevice) { // A系列此值为TAGGPS展示，不对EO和FOV产生影响
            ...
            cv::putText(frame0, ConvertTimesNum2Str(stSysStatus.eoValue) + "x", cv::Point(x + 30, y + 60), cv::FONT_HERSHEY_SIMPLEX, stSysStatus.osdFontSize, cv::Scalar(0, 255, 0), fontThickness, cv::LINE_AA);
