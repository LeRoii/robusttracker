...
realtracker::realtracker(std::string enginepath, std::string irEnginepath) : m_frameInfo()
{
    ...
    m_detector = new idetector(const_cast<char *>(enginepath.c_str()), 3, 12);
    m_detector->init();
...
void realtracker::init(const cv::Point &pt, cv::Mat image,bool isIrImg)
{
...
#else
    runDetectorNoDraw(image, detRet,isIrImg);
#endif
...
void realtracker::FSM_PROC_SEARCH(cv::Mat &frameDetect, cv::Mat &frameTracker)
{
    printf("\nSM_PROC_SEARCH\n");
...
void realtracker::FSM_PROC_STRACK(cv::Mat &frameDetect, cv::Mat &frameTracker, bool isIrImg)
{
    printf("\nFSM_PROC_STRACK\n");
    std::vector<bbox_t> detRet;
    ...
    if (!m_irFrame)
    {
        ...
        runDetector(detimg, detRet);
#else
        runDetectorNoDraw(frameDetect, detRet, isIrImg);
#endif
    }

    runTracker(frameTracker);
    // #if TRACKER_DEBUG
    //     auto detimg = frame.clone();
    //     runDetector(detimg, detRet);
    // #else
    ...
    else
    {
        if (m_irFrame)
        {
            ...
            rectangle(frameTracker, m_strackerRet, cv::Scalar(255, 255, 255), 3, 8);
            m_state = EN_TRACKER_FSM::STRACK;
            return;
        }
        for (int i = 0; i < detRet.size(); ++i)
        {
            ...
            {
                m_state = EN_TRACKER_FSM::DTRACK;
                m_trackObj.init(detRet[i], frameTracker);
            ...
            if (area > 0)
            {
                ...
                double sim = calculateHistogramSimilarity(frameTracker(brect), frameTracker(strackerRect));
...
void realtracker::FSM_PROC_DTRACK(cv::Mat &frameDetect, cv::Mat &frameTracker,bool isIrImg)
{
    printf("\nFSM_PROC_DTRACK\n");
    std::vector<bbox_t> detRet;
#if TRACKER_DEBUG
...
void realtracker::fsmUpdate(cv::Mat &frameDetect, cv::Mat &frameTracker, bool isIrImg)
{

    switch (m_state)
    {
    case EN_TRACKER_FSM::STRACK:
        FSM_PROC_STRACK(frameDetect, frameTracker, isIrImg);
        break;
    case EN_TRACKER_FSM::DTRACK:
        FSM_PROC_DTRACK(frameDetect, frameTracker, isIrImg);
        break;
    case EN_TRACKER_FSM::SEARCH:
        FSM_PROC_SEARCH(frameDetect, frameTracker);
        break;
    default:
        break;
    }
...
EN_TRACKER_FSM realtracker::update(cv::Mat &frameDetect, cv::Mat &frameTracker, std::vector<bbox_t> &detRet, uint8_t *trackerStatus, bool isIrImg)
{
    printf("realtracker::update start\n");

    fsmUpdate(frameDetect, frameTracker, isIrImg);
    ...
    if (m_state == EN_TRACKER_FSM::DTRACK || m_state == EN_TRACKER_FSM::STRACK)
    {
        ...
        if (!sevorKeepFlag)
        {
            x = m_trackObj.center().x - 960;
            y = m_trackObj.center().y - 540;
        }
...
void realtracker::runDetector(cv::Mat &frame, std::vector<bbox_t> &detRet, bool isIrImg)
{
...
void realtracker::runDetectorOut(cv::Mat &frame, std::vector<bbox_t> &detRet, bool isIrImg)
{
    // printf("realtracker::runDetector\n");
    std::vector<bbox_t> boxs;
...
void realtracker::runDetectorNoDraw(cv::Mat &frame, std::vector<bbox_t> &detRet, bool isIrImg)
{
    // printf("realtracker::runDetector\n");
    // std::vector<bbox_t> boxs;
    detRet.clear();
