...
#pragma omp parallel for
    ...
    if (!m_tracks.empty())
    {
...
#endif
        distMatrix_t costMatrix(N * M); //row = M, col = N
        ...
        //print costMatrix in a better way
        printf("%10c,", ' ');
        for(int i=0;i<N;i++)
        {
            char tmpstr[10];
        ...
        {
            char tmpstr[10];
            sprintf(tmpstr, "det[%d]", i);
            printf("%*s,", 10, tmpstr);

            for(int j=0;j<N;j++)
            {
                printf("%10.2f,", costMatrix[i*N+j]);
            }
            printf("\n");
        }
        // fflush(stdout);
...
#pragma omp parallel for
    ...
    {
        printf("ID:[%d], type:[%d], trace size:[%d], lostcnt:[%d]\n", m_tracks[i]->GetID(),m_tracks[i]->GetCurrType(),m_tracks[i]->GetTraceSize(),m_tracks[i]->SkippedFrames());
    }
#ifndef SILENT_WORK
        ...
        for (size_t j = 0; j < regions.size(); ++j)
        {
            ...
            if (m_settings.CheckType(m_tracks[i]->LastRegion().m_type, reg.m_type))
            {
				...
				if (m_settings.m_distType[ind] > 0.0f && ind == tracking::DistJaccard)
                {
                    ...
                    // std::cout << "reg.m_brect:" << reg.m_brect << std::endl;
                    if constexpr (DIST_LOGS)
            ...
            std::cout << "maxPossibleCost" << maxPossibleCost << std::endl; 
            }
