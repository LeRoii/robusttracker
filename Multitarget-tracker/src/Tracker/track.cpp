...
track_t CTrack::CalcDistJaccard(const CRegion& reg) const
{
    ...
    // std::cout << "m_lastRegion.m_brect:" << m_lastRegion.m_brect << std::endl;
    // printf("intarea:%f, unionarea:%f, iou:%f\n", intArea, unionArea, intArea / unionArea);
...
bool CTrack::IsLost()
{
    if(m_trace.size() < 5)
    {
        return m_skippedFrames > 1;
    }
    else if(m_trace.size() < 10)
    {
        return m_skippedFrames > 3;
    }
    else if(m_trace.size() < 25)
    {
        return m_skippedFrames > 5;
    }
    else
    {
