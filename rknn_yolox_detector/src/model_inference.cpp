...
int modelInference::nms(int validCount, std::vector<float> &outputLocations, std::vector<int> &class_id, std::vector<int> &order, float threshold, bool class_agnostic)
{
...
modelInference::modelInference(char *model_path_, int thread_index_)
{
...
int modelInference::detect()
{
    ...
    //     src = wrapbuffer_virtualaddr((void *)det_img.data, img_width, img_height, RK_FORMAT_RGB_888);
